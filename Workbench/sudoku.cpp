...
{
	fstream myfile;
	myfile.open("C://Users//Abbey//Desktop//sudoku.txt");
	cell myArray[9][9];
	int x = 0;
	int y = 0;
	char c;
	...
	do
	{
		...
		for (int n = 0; n < 9; n++)
		{
			for (int i = 0; i < 9; i++)
			{
				...
				if (counter == 0)
					cout << "error, number " << n << " impossible ";
				else if (counter == 1)
				{
					for (int k = 0; k < 9; k++)
					{
						if (k != n)
						{
							if (myArray[i][temp].number[k])
							{
								myArray[i][temp].number[k] = false;
								flag = true;
							}
						}
					}
			...
			for (int j = 0; j < 9; j++)
			{
				...
				else if (counter == 1)
				{
					for (int k = 0; k < 9; k++)
					{
						if (k != n)
						{
							if (myArray[temp][j].number[k])
							{
								myArray[temp][j].number[k] = false;
								flag = true;
							}
						}
					}
